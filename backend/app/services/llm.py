...
class LLMClient:
    ...
    def generate_answer(self, question: str, sources: list[SourceChunk]) -> str:
        if self.settings.openai_api_key:
            try:
                return self._generate_with_openai(question, sources)
            except Exception:
                return self._generate_local_answer(question, sources)
        return self._generate_local_answer(question, sources)

    def _generate_with_openai(self, question: str, sources: list[SourceChunk]) -> str:
